<script lang="ts">
	import { timeSelected } from '$lib/state/appointment';
	import {
		startOfMonth,
		endOfMonth,
		eachDayOfInterval,
	...
	import type { Package, Service } from '$lib/data/services';
	...
	const selectDate = (date: Date) => {
		if (service.calendarType === 'boarding' || service.calendarType === 'drop-in-walking') {
		...
		} else if (service.calendarType === 'training' && selectedPackage.visits) {
			// For training, you might allow multiple specific dates selection up to a limit
			if (!selectedDates.includes(date) && selectedDates.length < selectedPackage.visits) {
				selectedDates.push(date);
			}
		} else {
			selectedDate = date; // For services that only need one date
		}
	};
	...
	function generateTimeSlots(date: Date) {
		...
		const appointmentDates = existingAppointments.map((app) => {
			...
			return {
				...
				end: new Date(parseAppointmentDate(app.start).getTime() + selectedPackage?.duration * 60000)
			};
		});
		...
		while (startTime < endTime) {
			...
			if (!selectedPackage.duration) return;
			startTime = new Date(startTime.setMinutes(startTime.getMinutes() + selectedPackage.duration));
		}
	}
